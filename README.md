...
| `/irbid/` — **the landing page** (`irbid/index.html`, **generated**) | Google, Maps, anyone arriving from Instagram | the menu as real text with prices, set as their printed card; the `Restaurant` + `Menu` schema, Directions, Instagram. The whole page is one scroll in five scenes, all real material with nobody in frame: the lane walked into our red screen (`irbid/film/`), the OFA, the menu as the card, the milkshake, their lit sign at night (`irbid/scene/`). This is the Website and Menu link for the Google Business Profile, and the canonical URL of both pages |
