...
function mountScrollWorld(container, config) {
  ...
  SEGMENTS.forEach((s, i) => {
    ...
    // The opening poster is on screen the instant the engine mounts and is the
    // page's largest contentful paint - lazy-loading it defers the only pixels the
    // visitor has while the first (multi-MB) clip downloads. Later scenes stay lazy.
  ...
  SECTIONS.forEach((s, i) => {
    ...
    dot.setAttribute('aria-label', s.label || 'Scene ' + (i + 1));   // its visible label is hidden on a phone
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // The next segment fades in ABOVE the current one (z-130): with it below, its
      // opacity ramp happened entirely behind an opaque scene and every seam was a
      // hard cut whatever `crossfade` said — invisible only while the film is
      // frame-continuous. On top, the ramp is a real dissolve; continuous seams
      // blend near-identical frames and look unchanged.
      s.el.style.zIndex = (i === ci) ? '120' : (i === ci + 1) ? '130' : String(100 + Math.round(op * 10));
      // Deliberately no transform on the still: .sw-scene__video is never
      // transformed, so anything set here becomes a visible jump the moment the
      // clip takes over. A `translateX(2vw) scale(1.03+local*0.14)` here drew the
      // poster ~29px right and 3-17% large against its own frame 0 - defeating the
      // reason the posters ARE frame 0 - and under prefers-reduced-motion (scale 1,
      // still translated) it exposed a 29px strip of background down every left edge.
    }
...
function injectCSS() {
  ...
  .sw-brand{display:flex;align-items:center;gap:10px;min-height:44px;text-decoration:none;color:var(--sw-ink);}
  ...
  .sw-nav__item{font:inherit;font-size:.82rem;color:var(--sw-ink-soft);border:0;background:transparent;cursor:pointer;padding:7px 14px;min-height:44px;border-radius:999px;transition:color .25s,background .25s;}
  .sw-nav__item:hover{color:var(--sw-ink);} .sw-nav__item.is-active{color:var(--sw-bg);background:var(--sw-ink);}
  ...
  /* the dot is drawn at 14px (28 on a phone); the area a finger gets is 44 either way */
  .sw-route__dot::after{content:"";position:absolute;inset:-15px;}
  ...
  /* pointer-events:none: this stays in the DOM at opacity 0 for the whole page,
     and opacity/visibility do not stop hit-testing - without it the box is an
     invisible click-eater pinned to the bottom-centre of the viewport. */
  .sw-hint{position:fixed;left:50%;bottom:26px;z-index:30;pointer-events:none;transform:translateX(-50%);display:flex;flex-direction:column;align-items:center;gap:10px;font-size:.76rem;letter-spacing:.14em;text-transform:uppercase;color:var(--sw-ink-soft);transition:opacity .3s;}
